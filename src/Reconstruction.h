...
public:
	...
	std::vector<Eigen::Matrix3d> F_Me2Neighbors; //元素顺序和neigiborID要一一对应
...
public:
	...
	bool findTrack(uint ViewID, std::vector<Eigen::Matrix3d> F_neighbor2this, PointTrack& fromPointTrack); //一个点在所有neighbor中的track	
	void findTracksInViews();
	void GetStructure();
	void SavePLYFile_PointCloud(std::string filePath);
	void SavePLYFile_Mesh(std::string filePath);
	...
	void CalculateStructure_FirstXThenRWithScale(std::map<uint, Eigen::Matrix<double, 3, 4>>& map_RtMatrix, std::map<uint, Eigen::Matrix<double, 3, 4>>& map_PMatrix);
